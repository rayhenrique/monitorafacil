...
echo "==> [1/17] Atualizando repositório a partir da branch main..."
git pull origin main

echo "==> [2/17] Instalando dependências de produção do Composer..."
composer install --no-dev --optimize-autoloader

echo "==> [3/17] Compilando assets do frontend (Vite)..."
npm run build
...
echo "==> [4/17] Executando migrações do banco de dados..."
$PHP_BIN artisan migrate --force

if [ "$SYNC_DATA" = true ]; then
    echo "==> [5/17] Sincronizando dados nominais do PEC (CVAT Relação Nominal)..."
    ...
    echo "==> [6/17] Consolidando dados reais do Indicador C1 (Mais Acesso)..."
    ...
    echo "==> [7/17] Consolidando dados reais do Indicador C2 (Desenvolvimento Infantil)..."
    ...
    echo "==> [8/17] Consolidando dados reais do Indicador C3 (Gestação e Puerpério)..."
    ...
    echo "==> [9/17] Consolidando dados reais do Indicador C4 (Pessoas com Diabetes)..."
    ...
    echo "==> [10/17] Consolidando dados reais do Indicador C5 (Pessoas com Hipertensão)..."
    ...
    echo "==> [11/17] Consolidando dados reais do Indicador C6 (Cuidado da Pessoa Idosa)..."
    ...
    echo "==> [5-11/17] Sincronização de dados do PEC ignorada (--quick / --no-sync ativo)."
fi

echo "==> [12/17] Publicando assets do Livewire..."
$PHP_BIN artisan livewire:publish --assets

echo "==> [13/17] Limpando caches da aplicação..."
$PHP_BIN artisan optimize:clear

echo "==> [14/17] Otimizando cache de configuração..."
$PHP_BIN artisan config:cache

echo "==> [15/17] Reiniciando workers da fila com a versão nova..."
...
echo "==> [16/17] Otimizando cache de rotas..."
$PHP_BIN artisan route:cache

echo "==> [17/17] Otimizando cache de views..."

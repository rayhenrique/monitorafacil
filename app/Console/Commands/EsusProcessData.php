...
#[Signature('esus:process-data {--scope=all : Escopo de processamento: all (geral completo), c1 (indicador C1), c2 (indicador C2), c3 (indicador C3), c4 (indicador C4) ou c5 (indicador C5)} {--year= : Ano de competência (padrão: atual)} {--quarter= : Quadrimestre de competência (padrão: atual)}')]
...
class EsusProcessData extends Command
{
    public function handle(EsusDataProcessingService $service): int
    {
        ...
        if (! in_array($scope, ['all', 'c1', 'c2', 'c3', 'c4', 'c5'], true)) {
            $scope = 'all';
        }

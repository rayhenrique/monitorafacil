...
- **REGRA DE RODAPÉ ÚNICO (NUNCA DUPLICAR O RODAPÉ):** O rodapé da aplicação é exclusivo e renderizado globalmente pelo layout em `resources/views/layouts/app.blade.php`, contendo a mensagem institucional e a versão do sistema (`VersionService::CURRENT_VERSION`). **Nunca** insira rodapés internos em páginas, views filhas ou componentes Livewire/Blade. Onde for identificada duplicidade de rodapé, remova-a imediatamente mantendo apenas o rodapé do layout.
